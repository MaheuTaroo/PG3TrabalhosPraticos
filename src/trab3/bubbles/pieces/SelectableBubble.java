...
public abstract class SelectableBubble extends AbstractBubble {
	enum Dir {
		...
		Dir( int dl, int dc) {deltaLine = dl ; deltaColumn = dc; }
	};

	private boolean selected = false;
	public SelectableBubble(Board b, int l, int c ) { super(b, l, c);                 }

	public final boolean isSelected() { return selected;                              }
	public void unselect()            { selected = false; board.unselected( this ); }
	public void select()              { setSelected();                                }
	public boolean selectIf( Predicate<Bubble> pred ) {
		if ( !isSelected() && pred.test( this )) {
			setSelected();
			selectContiguos( pred );
			return true;
		}
		return false;
	}
